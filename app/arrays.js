arraysAnswers = {
  ...
  sum: function sum(arr) {
        return arr.reduce(function getSum(total, num) {return total + num;}, 0);
  },
  ...
  remove: function remove(arr, item) {
        return arr.filter(function exclude(element) { return element !== item;});
  },
...
//   },
  ...
  duplicates: function duplicates(arr) {
        let result = arr.filter(function(value,index,self){ return (self.indexOf(value) !== index )})
  ...
  findAllOccurrences: function findAllOccurrences(arr, target) {
        return arr.map((element, index) => element === target ? index : null).filter(element => element !== null);
  },
};
